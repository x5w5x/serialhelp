...
export async function connectSerial() {
    ...
    // if(Auto.isAutoEn&& !Auto.isDisCon &&(error.name === 'NetworkError' || error.name === 'NotFoundError')){
    //     await ConnectErro();
    // }else{
...
// async function ConnectErro() {
//     if(Auto.reCount<Auto.maxReCount){
//         Auto.reCount++;
//         debugLogMessage(`尝试重连(${Auto.reCount}/${Auto.maxReCount})...`);
//         await new Promise(re=>setTimeout( re,Auto.reDelay));
//         await connectSerial();
//     }else{
//         debugLogMessage("以达到最大重试次数，停止自动重连");
//         await cleanupSerialResources();
//         Auto.predPort=null;
//     }
// }

// async function diserro() {
//     if(!Auto.isDisCon&&Auto.isAutoEn){
//         debugLogMessage("检测到设备断开，尝试自动重连");
//         await cleanupSerialResources();
//         await connectSerial();
//     }else{
